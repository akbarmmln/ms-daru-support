  ...
  async function connect() {
    if (!format.isEmpty(clientId)) {
      ...
      wsInstance.on('open', () => {
        ...
        wsInstance.send(
          JSON.stringify({ type: 'register', agent: 'microservice', clientId, additional })
        );
      });
      ...
      logger.infoWithContext('This pod cannot run as a WebSocket client (2)');
    }
  }
...
async function waitForWebSocket() {
  if (wsInitialized) return wsInstance;
...
module.exports = {
  connectClientWS,
  waitForWebSocket,
  getWebSocket: () => wsInstance, // Optional direct access, but might still be null if accessed too soon
  WebSocket,
  clientId: () => clientId,
};
